...
pedidos.json (para o teste o mais aplicável era a utilização desse arquivo, para um desenvolvimento mais sério, utilizaria MongoDB)
...
- [Tree do Backend](#backend)
- [Get Route](#get-route)
- [Get Route ID](#get-route-id)
- [POST Route](#post-route)
- [DELETE Route](#delete-route)
- [PATCH Route](#patch-route)
- [PUT Route](#put-route)
- [Tree do Frontend](#frontend)
- [Docker](#docker)
- [Como Executar](#como-executar)
...
- Redireciona para o app.js que redireciona para o gerenciador de rotas correto, '/orders' e desse para a função especifica do controller;
...
- O filtro retorna os pedidos e retorna a requisição;
...
- O server.js recebe a requisição na porta 3000;
- Redireciona para o app.js que redireciona para o gerenciador de rotas correto, '/orders' e desse para a função especifica do controller;
- O orderController.js verifica se foi passado o id como parametro, retorna erro caso não tenha sido passado;
- O controller aciona o fileService.js para ler o arquivo pedidos.json e retornar os pedidos registrados e procurar pelo pedido usando o id, retorna erro caso não tenha encontrado;
...
- Caso retorne false, o fluxo é interrompido e retorna erro. Retornando true, a estrutura está correta, ele constroi a estrutura final do pedido no newOrderConstruction.js;
- Retornando a estrutura final, o fileService é acionado para o salvamento, e retorna a mensagem final da requisição;
...
- O server.js recebe a requisição na porta 3000;
- Redireciona para o app.js que redireciona para o gerenciador de rotas correto, '/orders' e desse para a função especifica do controller;
- O controller verifica se o id, e o status (recebido no body da requisição), foram recebidos, retorna erro se não;
...
- Se o novo status for inválido retorna erro, se for valido salva no fileService.js;
...
- O server.js recebe a requisição na porta 3000;
- Redireciona para o app.js que redireciona para o gerenciador de rotas correto, '/orders' e desse para a função especifica do controller;
- O controller recebe, somente, os campos que podem ser atualizados: customer, delivery_address, payments. E verifica se o id foi passado como parametro, retorna erro se não;
...
- Caso os campos enviados não recebam autorização para serem atualizados, retorna erro. Caso recebam a permissão, o controller aciona orderUpdaterSchemaValidation.js para fazer a validação da estrutura de cada campo. Exemplo:
...
- Retorno inválido: retorna erro. Retorno válido, o controller pega o pedido a ser atualizado juntamente com os novos dados e envia para o orderUpdater.js para montar o novo pedido;
- Após a montagem, o pedido atualizado é juntado com os outros e enviado para o fileService para o salvamento;
...
- Antes de começar, você precisa ter instalado:
    ...
    * Docker
    * Docker compose - atualmente já incluso no Docker Desktop (Win/Mac) ou docker-compose-plugin (Linux)
    * Git
...
4. **Acesse a aplicação**
   ...
   - Frontend: abra o navegador em http://localhost:8080
...
(É necessário reiniciar a sessão após executar este comando)
...
- ✅ Títulos padronizados com a formatação correta (#, ##, ###)
- ✅ Sumário criado com links para todas as seções
- ✅ Atalhos criados com links para os tópicos principais
- ✅ Códigos formatados em blocos ```json e ```bash
- ✅ Listas organizadas com * e -
- ✅ Links funcionando (#nome-da-seção)
- ✅ Conteúdo mantido exatamente como você escreveu, sem alterações
